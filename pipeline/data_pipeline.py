...
def image_generator(case, batch_size=32):
    ...
    if case ==7:
        ...
        for file in files_1[:len(files_1)//2]:
            train_df = train_df.append({'filename': os.path.join(appendpath,file), 'class': 0}, ignore_index=True)
        for file in files_2[:len(files_2)//2]:
            train_df = train_df.append({'filename': os.path.join(appendpath,file), 'class': 1}, ignore_index=True)
