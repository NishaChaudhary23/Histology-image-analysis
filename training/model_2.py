...
base_path = "/storage/bic/data/oscc/project_3"
train_large = '/storage/bic/data/oscc/project_3/dataset-wsi'


#reading & displaying an image
a = np.random.choice(['normal','osmf','oscc'])
path = '/storage/bic/data/oscc/project_3/dataset-wsi/{}/'.format(a)
...
if not os.path.exists(f'{base_path}/models/{model_type}'):
        os.makedirs(f'{base_path}/models/{model_type}')
# Model Summary
...
# Saving the model
model.save(f'{base_path}/models/{model_type}/{model_type}.h5')
print("------------------------------------------")
print(f'Model saved')
print("------------------------------------------")
...
plt.tight_layout()
plt.savefig(f'{base_path}/models/{model_type}/Accuracy.jpg')

# np.save('{base_path}/models/{model_type}/history1.npy',history.history)

hist_df = pd.DataFrame(history.history) 

# save to json:  
hist_json_file = f'{base_path}/models/{model_type}/history.json' 
with open(hist_json_file, mode='w') as f:
    hist_df.to_json(f)

# or save to csv: 
hist_csv_file = f'{base_path}/models/{model_type}/history.csv'
with open(hist_csv_file, mode='w') as f:
    hist_df.to_csv(f)

loaded_model = load_model(f'{base_path}/models/{model_type}/{model_type}.h5')
...
plt.tight_layout()
plt.savefig(f'{base_path}/models/{model_type}/Confusion_matrix.jpg')

conf_df = pd.DataFrame(confusion, index = ['normal','osmf','oscc'], columns = ['normal','osmf','oscc'])
conf_df.to_csv(f'{base_path}/models/{model_type}/Confusion_matrix.csv')
...
df.to_csv(f'{base_path}/models/{model_type}/Classification_report.csv')
